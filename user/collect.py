...
class SearchThread(QThread):
    ...
    def run(self):
        # 查询数据库获取数据
        query = "SELECT * FROM book_collect_table WHERE id = %s"
...
class collect(QMainWindow):
        ...
        if self.db:
            try:
                ...
                if result[0] != 0:
                    ...
                    # 设置表格的列数和对应的标题文字
                    self.table_widget.setColumnCount(9)  # 有9列
                    column_labels = ["用户id", "书名", "作者", "介绍", "第一大类", "第二大类", "图书编号", "预约", "删除"]
    ...
    def add_row_to_table(self, row_data):
        ...
        self.table_widget.setCellWidget(current_row, 8, book_delete)  # 最后一列
        ...
        self.table_widget.setCellWidget(current_row, 7, book_reserve)  # 倒数第二列
